...
def load_user_data(user_email: str):
    """Load saved messages + topic history for this user from Firestore."""
    try:
        db = get_db()
        doc = db.collection("study_history").document(user_email).get()
        if doc.exists:
            data = doc.to_dict()
...
def save_user_data(user_email: str, messages: list, topics: list, force: bool = False):
    ...
    try:
        db = get_db()
        doc_ref = db.collection("study_history").document(user_email)
...
if "messages" not in st.session_state:
    loaded_messages, loaded_topics = load_user_data(user_email)
...
# ── Sidebar: study history + sign out ────────────────────────────────────────────
with st.sidebar:
    ...
    with col1:
        if st.button("🎨 Visualize", use_container_width=True, key="viz_btn"):
            ...
            if diagram:
                ...
                save_user_data(user_email, st.session_state.messages, st.session_state.topics)
                st.rerun()
            else:
