...
def analyze_uploaded_image(image_bytes, mime_type, caption, level):
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    f"always attempt a genuinely helpful explanation. Answer directly without "
                    f"lengthy internal reasoning. IMPORTANT: keep your entire response under "
                    f"300 words — cover only the most important points, don't try to describe "
                    f"every single label or detail if the image is complex."
                )},
                {"role": "user", "content": [
        ...
        # If stripping <think> tags left nothing, fall back to the raw text
