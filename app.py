...
def generate_mermaid_code(topic, level):
    """Ask Groq to write a short Mermaid flowchart/mindmap for the topic."""
    client = groq.Groq(api_key=groq_key)
    try:
        response = client.chat.completions.create(
            model="openai/gpt-oss-120b",
            messages=[
                {"role": "system", "content": (
                    "You write Mermaid diagram syntax only. Output ONLY valid Mermaid code — "
                    "no markdown fences, no explanation, no extra text before or after. "
                    "Use 'flowchart TD' or 'mindmap' syntax. Use 4-8 short nodes with plain, "
                    "simple labels (2-5 words each). Do NOT use quotes, parentheses, colons, or "
                    "special characters inside node labels — plain words and spaces only."
                )},
                {"role": "user", "content": f"Create a simple flowchart or mindmap explaining: {topic}\nDifficulty level: {level}\nOutput only the mermaid code, nothing else."}
            ],
            temperature=0.4,
            max_tokens=400
        )
        code = clean_mermaid_code(response.choices[0].message.content)
        if re.match(r'^(flowchart|graph|mindmap)\b', code, re.IGNORECASE):
            return code
    except Exception:
        pass
    return None
...
def generate_diagram(topic):
    """Try a real labeled diagram via Mermaid.ink first; fall back to a
    symbolic Flux image if the AI-written Mermaid syntax fails to render."""
    level = st.session_state.get("level", "Student")
    mermaid_code = generate_mermaid_code(topic, level)
    if mermaid_code:
        img = render_mermaid(mermaid_code)
        if img:
            return img
    # Fallback — Mermaid syntax generation or rendering failed
