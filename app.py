...
if st.session_state.get("just_created"):
    st.success(f"✅ Bookmark this page now — it will bring you straight back to this session. Your code is **{st.session_state.session_code}** as a backup.")
    st.session_state.just_created = False

    with st.expander("✏️ Want an easier code to remember? Rename it here"):
        custom_code = st.text_input("Custom session code", placeholder="e.g. MY-STUDY-CODE", key="custom_code_input")
        if st.button("Save custom code", key="save_custom_code_btn"):
            cleaned = custom_code.strip().upper().replace(" ", "-")
            if not cleaned:
                st.warning("Type a code first.")
            elif code_exists(cleaned):
                st.error("That code is already taken — try a different one.")
            else:
                old_code = st.session_state.session_code
                st.session_state.session_code = cleaned
                st.query_params["code"] = cleaned
                # carry over any messages/topics already loaded under the old code
                save_user_data(cleaned, st.session_state.get("messages", []), st.session_state.get("topics", []), force=True)
                st.rerun()
...
def generate_diagram(topic):
    ...
    prompt = (
        f"A simple, clean conceptual illustration representing the idea of: {topic}. "
        "Minimalistic flat vector-style illustration, no text, no words, no letters, "
        "no labels, white background, soft colors, icon-like, symbolic and visual only."
    )
